...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/codex-companion.mjs task [--background] [--write] [--isolated] [--cwd <dir>] [--resume-last|--resume|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [prompt]",
...
function buildTaskRequest({ cwd, model, effort, prompt, write, isolated, resumeLast, jobId }) {
  return {
    cwd,
    model,
    effort,
...
async function handleTask(argv) {
  const { options, positionals } = parseCommandInput(argv, {
    ...
    booleanOptions: ["json", "write", "isolated", "resume-last", "resume", "fresh", "background"],
    aliasMap: {
      m: "model"
    }
  });
